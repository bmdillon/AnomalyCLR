...
def add_objects (batch):
    ...
    for n in range( batch_filled.shape[0] ):
        ...
        els = np.concatenate( [el_pts, el_phis, el_etas, el_one_hot], axis=1 )
        ...
        mus = np.concatenate( [mu_pts, mu_phis, mu_etas, mu_one_hot], axis=1 )
        ...
        jets = np.concatenate( [jet_pts, jet_phis, jet_etas, jet_one_hot], axis=1 )
...
def add_objects_constptmet( batch, scaler_pt, scale_angle, etaphi_smear_strength ):
    batch_filled = batch.copy()
    batch_filled = collinear_fill_jets( batch_filled )
    batch_filled = collinear_fill_e_mu( batch_filled )
...
def neg_augs( batch, scaler_pt, scale_angle, etaphi_smear_strength, addobj=True, addobj_wcpm=True, shpt=True, shmet=True, shporm=False ):
    ...
    for j in range( n_augs ):
        aug = aug_list[j]
        n = rand_opts[j]
        if aug=="ao":       batch_aug[ np.where(rands==n) ] = add_objects( batch_aug[ np.where(rands==n) ] )
